...
class SMPLX_ALL(nn.Module):
    ...
    def __init__(self, batch_size=1, use_face_contour=True, all_gender=False, **kwargs):
        ...
        self.model_dict = nn.ModuleDict({gender: SMPLX(path_config.SMPL_MODEL_DIR,
                                                       ...
                                                       ext='npz',
                                                       num_betas=numBetas,
                                                       use_pca=False, batch_size=batch_size,
                                                       use_face_contour=use_face_contour, num_pca_comps=45, **kwargs)
                                         for gender in self.genders})
        ...
        self.smplx2flame = torch.from_numpy(
            np.load(os.path.join(SMPL_MODEL_DIR, 'model_transfer/SMPL-X__FLAME_vertex_ids.npy'))).long()
        ...
        limb_idx = [int(i) for i in self.smplx2flame] + [int(i) for i in self.smplx2lhand] + [int(i) for i in
                                                                                              self.smplx2rhand]
        smplx2body = []
    ...
    def get_tpose(self, betas=None, gender=None):
        ...
        for gi, g in enumerate(['male', 'female', 'neutral']):
            ...
            J = getattr(self, f'{g}_J_template').unsqueeze(0) + blend_shapes(gender_kwargs['betas'],
                                                                             getattr(self, f'{g}_J_dirs'))
...
def get_smpl_tpose():
    smpl = SMPL(create_betas=True, create_global_orient=True, create_body_pose=True, model_path=SMPL_MODEL_DIR,
                batch_size=1)
...
def get_partial_smpl(body_model='smpl', device=torch.device('cuda')):
    ...
    for part in ['lhand', 'rhand', 'face', 'arm', 'forearm', 'larm', 'rarm', 'lwrist', 'rwrist']:
        part_vid_fname = 'data/{}_{}_vids.npz'.format(body_model, part)
        if os.path.exists(part_vid_fname):
            ...
            if part in ['lhand', 'rhand']:
                ...
                with open(os.path.join(SMPL_MODEL_DIR, '{}_vert_segmentation.json'.format(body_model)),
                          'rb') as json_file:
                ...
                if part == 'face':
                    ...
                    selected_body_part = ['rightHand', 'leftArm', 'leftShoulder', 'rightShoulder', 'rightArm',
                                          'leftHandIndex1', 'rightHandIndex1', 'leftForeArm', 'rightForeArm',
                                          'leftHand', ]
                    # selected_body_part = ['rightHand', 'leftArm', 'rightArm', 'leftHandIndex1', 'rightHandIndex1', 'leftForeArm', 'rightForeArm', 'leftHand',]
                elif part == 'forearm':
                    selected_body_part = ['rightHand', 'leftHandIndex1', 'rightHandIndex1', 'leftForeArm',
